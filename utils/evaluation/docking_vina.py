...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, pred_protein_path=None,
                           protein_root='./apo2mol_dataset/data_folder', pocket_type='holo',
                           **kwargs):
        # load original pdb
        if 'apo2mol' in protein_root:
            if pocket_type == 'holo':
                ...
                protein_path = os.path.join(protein_root, protein_fn)
            elif pocket_type == 'gen':
                protein_path = pred_protein_path
